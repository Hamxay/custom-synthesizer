...
class ElevenLabsWSSynthesizer(
    ...
    def __init__(
        ...
        self.text_chunk_queue: asyncio.Queue[Optional[BotBackchannel | LLMToken]] = (
            asyncio.Queue()
        )
        ...
        self.sample_width = (
            2 if synthesizer_config.audio_encoding == AudioEncoding.LINEAR16 else 1
        )
    ...
    async def establish_websocket_listeners(self, chunk_size):
        ...
        async with websockets.connect(
                ...
                while True:
                    ...
                    if response.audio:
                        ...
                        if response.alignment:
                            ...
                            self.current_turn_utterances_by_chunk.append(
                                (utterance_chunk, seconds)
                            )
                        ...
                        if first_message and backchannelled:
                            ...
                            logger.info(
                                "First message was a backchannel, reducing volume."
                            )
                            reduced_amplitude_buffer = self.reduce_chunk_amplitude(
                                buffer,
                                factor=self.synthesizer_config.backchannel_amplitude_factor,
                            )
                            ...
                            for chunk_idx in range(
                                0, len(buffer) - chunk_size, chunk_size
                            ):
            ...
            self.websocket_functions = await asyncio.gather(
                *self.websocket_tasks.values()
            )
